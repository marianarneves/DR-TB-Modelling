...
#DT ALone
# Chance Node
C_FLQ_cost_DTalone = ChanceNode(name='C_FLQ_cost_DTalone', cost=Cost_FLQ, future_nodes=[C_FLQsus_cost, C_FLQres_cost],
                              probs=[(1-FLQ_Res_prev), FLQ_Res_prev])
D_cost_DTalone = DecisionNode(name='D_cost_DTalone', cost=0, future_nodes=[C_FLQ_cost_DTalone, C_DLM_cost])
Exp_FLQ_cost_DTalone = D_cost_DTalone.get_expected_costs()['C_FLQ_cost_DTalone']
Exp_DLM_cost_DTalone = D_cost_DTalone.get_expected_costs()['C_DLM_cost']
...
for DALY_Death, obs in zip(DALY_individual_Moldova, pred_data['obs']):
    ...
    ##### DT Alone ####
    C_FLQ_DALY_DTalone = ChanceNode(name='C_FLQ_DALY_DTalone', cost=0, future_nodes=[C_FLQsus_DALY, C_FLQres_DALY],
                              probs=[(1-FLQ_Res_prev), FLQ_Res_prev])
    D_DALY_DTalone = DecisionNode(name='D_FLQ_DALY_DTalone', cost=0, future_nodes=[C_FLQ_DALY_DTalone, C_DLM_DALY])
    Exp_FLQ_DALY_DTalone = D_DALY_DTalone.get_expected_costs()['C_FLQ_DALY_DTalone']
    Exp_DLM_DALY_DTalone = D_DALY_DTalone.get_expected_costs()['C_DLM_DALY']
    #NMB
    NMB_DTalone = GBD_moldova*(Exp_FLQ_DALY_DTalone - Exp_DLM_DALY_DTalone) - (Exp_DLM_cost_DTalone - Exp_FLQ_cost_DTalone)

    if NMB_DTalone > 0:
        Cost_DTalone.append(Exp_DLM_cost_DTalone)
        DALY_DTalone.append(Exp_DLM_DALY_DTalone)
    else:
        if obs == 1:
            Cost_DTalone.append(Exp_FLQ_res_cost)
            DALY_DTalone.append(Exp_FLQres_DALY)
        else:
            Cost_DTalone.append(Exp_FLQ_sus_cost)
            DALY_DTalone.append(Exp_FLQsus_DALY)


    ############ End Calculation ############

Cost_DTalone_individual = sum(Cost_DTalone)/540
DALY_DTalone_individual = sum(DALY_DTalone)/540
print(sum(Cost_DTalone)/540)
print(sum(DALY_DTalone)/540)
...
plt.grid(True)
#plt.savefig(r'/Users/mraniereneves/Library/CloudStorage/OneDrive-YaleUniversity/Yale/TB/DR-TB-Modelling/Cost-effectiveness/Decision Trees/Extended Tree - XDR-TB/Output/NMB_St_Treat_Opt_Treat_Individual.png')
plt.show()
...
Increase_DTalone_Cost = Cost_DTalone_individual- FLQ_Cost
Increase_DTalone_DALY = FLQ_DALY - DALY_DTalone_individual
...
plt.scatter(Increase_DTalone_DALY, Increase_DTalone_Cost,  marker='s', s=100, color='green', label='DT alone')
plt.scatter(0, 0, color='red', label='Standard Treatment')
