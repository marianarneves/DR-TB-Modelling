...
class DRTuberculosisDT:
    ...
    def optimal_treat_pmdt_sample(self, pt, prediction, t, wtp, decisiontree_prob_df):
        ...
        for j in range(1, self.par_samplesize + 1):
            ...
            Recurrence_FLQsus = TerminalNode(name='Recurrence_FLQsus', cost=prob_cost_daly_sampled['Cost_Recurrence_FLQsus'],
    ...
    def optimal_treat_dt_sample(self, pt, wtp):
        ...
        for j in range(1, self.par_samplesize + 1):
            prob_cost_daly_sampled = self.par_sampler.cost_daly_input_table(self.prob_sampled_par,
                                                                                 self.cost_sampled_par,
                                                                                 self.dalyweight_sampled_par,
                                                                                 self.dalylength_sampled_par,
                                                                                 self.sideeffectdaly_sampled_par,
            ...
            D = DecisionNode(name='D', cost=0, daly=0, future_nodes=[C_FLQ, CC_DLM])
    ...
    def dr_tb_pmdt_s_opt(self, allthresholds, DALY_individual_Moldova, preliminary_daly_costs, correction="None", sample_disease_prev = "N"):
        ...
        if (correction == "OptCorr_Adj"):
            ...
            for i in range(1, self.par_samplesize + 1):
                ...
                df_results_bootstrap = self.pm_performance.evaluate_pm_thresholds(allthresholds, "else","bootstrap",
                                                           bootstrapsample=i)
                df_results_origidata = self.pm_performance.evaluate_pm_thresholds(allthresholds, "else", "origdata",
                                                           bootstrapsample=i)
                # Update the DataFrame using the replacement function
                df_results_bootstrap_updated = self.pm_performance.replace_rows_with_min_classes(df_results_bootstrap, minclass_perc)
                df_results_origidata_updated = self.pm_performance.replace_rows_with_min_classes(df_results_origidata, minclass_perc)
            ...
            bootstrap_results_bythreshold = self.pm_performance.extract_results_by_threshold(bootstrap_results, allthresholds)
            origidata_results_bythreshold = self.pm_performance.extract_results_by_threshold(origidata_results, allthresholds)
