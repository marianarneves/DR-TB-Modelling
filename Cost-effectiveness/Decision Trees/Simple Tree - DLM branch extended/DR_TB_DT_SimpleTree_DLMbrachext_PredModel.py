...
T2_p = TerminalNode(name='T2_p', cost=CostFLQ_DLM)
T3_p = TerminalNode(name='T3_p', cost=CostDLM)
T4_p = TerminalNode(name='T4_p', cost=CostDLM_FLQ)

# Negative tree
T1_n = TerminalNode(name='T1_n', cost=CostFLQ)
T2_n = TerminalNode(name='T2_n', cost=CostFLQ_DLM)
...
with open(csv_file, 'r') as file:
    ...
    for row in reader:
        ...
        C_DLM_p = ChanceNode(name='C_DLM_p', cost=0, future_nodes=[T3_p, T4_p], probs=[P_R_P, P_NR_P])
        # create D1
        D_p = DecisionNode(name='D_p', cost=0, future_nodes=[C_FLQ_p, C_DLM_p])
        # Calculating minimum cost
        if D_p.get_expected_costs()['C_FLQ_p'] > D_p.get_expected_costs()['C_DLM_p']:
            C_p = D_p.get_expected_costs()['C_DLM_p']
        else:
        ...
        C_DLM_n = ChanceNode(name='C_DLM_n', cost=0, future_nodes=[T3_n, T4_n], probs=[P_R_N, P_NR_N])
        # create D1
        D_n = DecisionNode(name='D_n', cost=0, future_nodes=[C_FLQ_n, C_DLM_n])
        # Calculating minimum cost
        if D_n.get_expected_costs()['C_FLQ_n'] > D_n.get_expected_costs()['C_DLM_n']:
            C_n = D_n.get_expected_costs()['C_DLM_n']
        else:
...
C_Std_Treat = ((1-FLQ_R_prev)*CostFLQ + FLQ_R_prev*CostFLQ_DLM) * 540 # Cost of standard treatment
